      ...
      <div className="flex items-center justify-center w-screen h-screen bg-gray-100">
        {chatActive ? (
          <div className="w-full rounded-md md:w-[80vw] lg:w-[40vw] p-2">
            <h1 className="my-2 text-xl font-bold text-center uppercase">Squad Chat</h1>
            <div>
              <div className="overflow-scroll h-[80vh] lg:h-[60vh]">
                {messages.map((message, index) => {
                  return (
                    <div key={index} className={`flex my-5 rounded-md shadow-md w-fit ${username === message.user && "ml-auto" } `}>
                      <div className="flex items-center justify-center bg-green-400 rounded-l-md">
                        <h3 className="px-2 text-lg font-bold ">{message.user.charAt(0).toUpperCase()}</h3>
                      </div>
